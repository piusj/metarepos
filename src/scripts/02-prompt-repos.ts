...
export async function promptRepos(
  ...
  console.log(
    chalk.dim(
      "  First, link existing repos from a parent directory.\n" +
  ...
  const candidates = (await listSubdirs(absoluteParent)).filter(
    (name) => name !== metarepoBase && !taken.has(name),
  );

  if (candidates.length === 0) {
    console.log(
      chalk.dim(
        `  No subdirectories found under ${chalk.bold(absoluteParent)} to symlink — you can still add repos individually below.`,
      ),
    );
  } else {
    ...
    if (selected.length > 0) {
      console.log(
        chalk.dim(
          `  ${selected.length} repo${selected.length === 1 ? "" : "s"} queued for symlinking.`,
        ),
      );
    }
  }
