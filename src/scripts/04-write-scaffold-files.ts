...
export async function writeScaffoldFiles(
  ...
  writes.push(
    await writeFileIfMissing(join(metarepoPath, "AGENTS.md"), agentsMd(name)),
  );
  writes.push(
    await writeFileIfMissing(join(metarepoPath, "CLAUDE.md"), claudeMd()),
  );
  writes.push(
    await writeFileIfMissing(
      join(metarepoPath, "META-ROOT.md"),
      metaRootMd(name),
    ),
  );
  writes.push(
    await writeFileIfMissing(join(metarepoPath, "README.md"), readmeMd(name)),
  );
  writes.push(
    await writeFileIfMissing(
      join(metarepoPath, ".gitignore"),
      gitignoreContent(),
    ),
  );
  writes.push(
    await writeFileIfMissing(
      join(metarepoPath, "META-ARCH-PROMPT.md"),
      metaArchPromptMd(),
    ),
  );
  writes.push(
    await writeFileIfMissing(join(metarepoPath, "repos/.gitkeep"), ""),
  );
