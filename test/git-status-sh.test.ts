...
async function gitCommit(dir: string, allowEmpty = false): Promise<void> {
  const args = ["-C", dir, "-c", "user.email=t@t", "-c", "user.name=t", "commit", "-q", "-m", "init"];
...
test("git-status.sh non-TTY output has no raw ANSI escape sequences", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(stdout, /\x1b\[/, "stdout leaked ANSI escapes in non-TTY mode");
  } finally {
...
test("git-status.sh renders the aligned-table header and divider", async () => {
  ...
  try {
    ...
    assert.match(stdout, /PROJECT\s+BRANCH\s+UPSTREAM\s+CHANGES/, "header row missing");
...
test("git-status.sh marks clean, dirty, and non-git entries with distinct bullets", async () => {
  ...
  try {
    ...
    assert.match(stdout, /not a git repo/, "non-git entry should carry guidance text");
    assert.match(stdout, /~1 modified/, "dirty repo should report modified count");
    assert.match(stdout, /\?1 untracked/, "dirty repo should report untracked count");
    assert.match(stdout, /Summary:.*with changes/, "summary line should appear");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
